<template>
  <div class="main-wrap-feedback">
    <div class="feedback-sec shadow-background">
      ...
      <div class="f-body">
        ...
        <div class="martop16">
          <FeedbackInfoCard :data="classDetails" />
        </div>
      </div>
    </div>
    <div class="martop32">
            ...
            <div class="martop16">
              <div class="subhead">
                <p>
                  Total:
                  <span>{{
                    processedDataWithoutComment[category].length
                  }}</span>
                </p>
              </div>
              <div class="main-wrap">
                <div
                  v-for="item in processedDataWithoutComment[category]"
                  :key="item.lfc_seq"
                  class="list"
                >
                  <p>{{ item.item_title }}</p>
                  <div class="points">
                    ...
                    >
                      ...
                      <div
                        v-else-if="
                          item.item_type == 2 &&
                          item.item_title != 'Overall Comments'
                        "
                      >
                        <textarea
    ...
    <div class="martop32">
      <div class="comment-sec">
        ...
        <textarea
          :key="finalOverallComment?.lfc_seq"
          :maxlength="250"
          @input="
          ...
          :id="
            finalOverallComment?.category_name + finalOverallComment?.item_title
          "
          :v-model="finalOverallComment.answer?.reply_txt"
  ...
  <ConfirmModalVue
    ...
    title="Would you like to save feedback LPE?"
    primaryButtonText="Save"
    secondaryButtonText="Cancel"
    @on-cancel="closeConfirmModal"
    @on-confirm="setLPEDetails"
  />
  <LoaderVue :loading="isLoading" />
</template>

<script setup>
import FeedbackInfoCard from "@/components/MyClass/TodaysClass/Feedback/FeedbackInfoCard.vue";
...
const processedDataWithoutComment = computed(() => {
  ...
  Object.keys(final).forEach((category) => {
    final[category] = final[category].filter((item, index) => {
      if (item.item_title == overAllItem) {
        return false;
      }
      return true;
    });
...
const overAllItem = "Overall Comments";
const finalOverallComment = computed(() => {
  ...
  Object.assign(processedData.value, processedData.value);
  Object.keys(processedData.value).forEach((category) => {
    final = processedData.value[category].find((item) => {
      if (item.item_title == overAllItem) {
        return true;
      }
    });
...
const getLPEDetails = async () => {
  ...
  try {
    ...
    const res = await feedback.getLPEDetails(data);
    if (res.data?.result) {
      lpeData.value = res.data;
      setData.value = res.data.lpeAnswerArray;
      processedData.value = res.data.lpeFormData?.reduce((final, current) => {
        if (!final[current.category_name]) final[current.category_name] = [];

        current.answer = res.data.lpeAnswerArray.find((answer) => {
          return (
            answer.lf_seq == current.lf_seq && answer.lfi_seq == current.lfi_seq
          );
        });
        current.case = res.data.lpeCaseArray.filter((answer) => {
          if (
...
const setLPEDetails = async () => {
  ...
  if (Object.keys(errors.value).length) {
    ...
    try {
      isLoading.value = true;
      const res = await feedback.setLPEDetails(data);
      if (res.data?.result) {
        await getLPEDetails();
      }
    } catch (error) {
